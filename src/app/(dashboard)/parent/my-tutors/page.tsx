      ...
      {!isLoading && connections.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 text-center px-4">
          <div className="w-20 h-20 bg-surface-hover rounded-full flex items-center justify-center mb-4">
            <UserSearch size={32} className="text-text-muted opacity-20" />
          </div>
          <h3 className="text-lg font-bold text-text-main">
            No tutors hired yet
          </h3>
          <p className="text-text-muted text-sm max-w-xs mx-auto">
            Review applications from your job posts to start connecting with
            tutors.
          </p>
        </div>
      )}

      {/* Infinite Scroll Trigger with Your Skeleton Logic */}
